...
Require Import List Omega std calculus higher_order_unification.
...
Section SystemUnification.
  ...
  Definition SU (I: sysuni) :=
    exists (Delta: ctx) (sigma: fin -> exp X), Delta ⊩ sigma : Gammaᵤ' /\ (sigma •₊ left_side Eᵤ') ≡₊ (sigma •₊ right_side Eᵤ').
...
(** ** Normalisation *)
Definition SNU {X: Const} (I: sysuni X) :=
  exists Delta sigma, Delta ⊩ sigma : Gammaᵤ' /\ (sigma •₊ left_side Eᵤ') ≡₊ (sigma •₊ right_side Eᵤ') /\
                 forall x, normal (sigma x).

Lemma SU_SNU X I: SU X I <-> SNU I.
Proof.
  split; intros (Delta & sigma & H1 & H2); [| exists Delta; exists sigma; intuition].
  ...
  + eapply equiv_pointwise_eqs; intros.
    ...
    all: eauto using left_typing, right_typing, @Hᵤ'. 
