...
Require Import Morphisms Omega List std calculus
        higher_order_unification systemunification
...
Section Conservativity.
  ...
  Proof.
    ...
    split; intros ? [x] % in_flat_map; eapply in_flat_map.
  ...
  Proof.
    intros Leq (Delta & sigma & T & H).
    unfold OU. eapply unification_downcast_eqs; eauto using @H₀'.
    intros ???; eapply ordertyping_soundness; eauto.  
  Qed.
  ...
  Proof.
    intros Leq (Delta & sigma & T & H).
    eapply unification_downcast_eqs; eauto using H₀'.
  Qed.
...
(** ** Consequences *)
Require Import third_order.pcp third_order.simplified third_order.huet.
