...
Require Import List Omega std unification calculus.
Import ListNotations.
Require Import second_order.diophantine_equations dowek.encoding.
...
Section EquationEquivalences.
  ...
  Section Constants.
    ...
    Lemma forward_consts:
      n = c -> sigma • fst (constEQ x c) ≡ sigma • snd (constEQ x c).
    Proof.
    ...
    Lemma backward_consts:
      sigma • fst (constEQ x c) ≡ sigma • snd (constEQ x c) -> n = c.
    Proof.
...
Section Forward.
  ...
  Proof.
    ...
    now eapply forward_typing. eapply equiv_pointwise_eqs.
    intros s t H'; cbn.
...
Section Backward.
  ...
  Proof.
    ...
    exists (sub_sol tau). specialize (equiv_eqs_pointwise _ _ EQ) as EQ'.
    assert (forall e, e ∈ Eqs E -> tau • fst e ≡ tau • snd e) as H by now intros []; eauto. clear EQ'.
