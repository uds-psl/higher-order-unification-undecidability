...
Require Import List Lia Arith Wf Morphisms std unification calculus concon.retyping.
Import ListNotations.

Tactic Notation "simplify" := HOU.std.tactics.simplify.  
...
Section Update.
  ...
  Proof.
    ...
    eapply varof_vars in H2; intuition.
    inv H2. eapply varof_vars in H1; intuition.
  Qed.
  ...
  Proof.
    ...
    repeat eapply incl_app_build; intuition.
  Qed.
...
Section LambdaFreeness.
  ...
  Proof.
    ...
    - eapply AppR_ordertyping_inv in H0 as H4; destruct H4 as [L H4].
      ...
      + destruct s; cbn in i; intuition.
        * inv H3. simplify in H6. lia. 
        * inv H3. rewrite H6 in H7.
          simplify in H7. lia.
      + eapply orderlisttyping_element in H2 as [B []]; eauto.
        eapply H; eauto.
...
Section Unification.
  ...
  Section Decomposition.
    ...
    Proof.
      ...
      - inv H4. inv H5. inv H6. inv H7.
        ...
        destruct (head s1); cbn in *; intuition; [| destruct (head t1)]; cbn in *; intuition.
        ...
        eapply app_injective_right in H8; intuition. congruence. 
    Qed.
    ...
    Proof.
      decomp_ind; cbn; intuition; simplify.
      rewrite H1, H3. (repeat eapply incl_app_build); intuition. 
    Qed.
  ...
  Section Computability.
    ...
    Lemma unifiy_computable E:
      { sigma | E ↦ sigma } + ({ sigma | E ↦ sigma } -> False).
    Proof with try solve [right; intros [sigma H]; inv H; intuition; congruence].
      induction E as [E IH] using eqs_size_induction .
      destruct (decomp' E) as [[| [e s] E']|] eqn: DE...
      - left; exists var; now econstructor. 
      - destruct e as [x | | |]...
        destruct (x el vars s) as [?|H1]...
        destruct (isFree x)...
        edestruct dec_all with (P := free) (x := vars s); eauto.
        2: right; intros [sigma H]; inv H;
          [congruence|]; rewrite DE in *; injection H0 as ???; subst; intuition.
        eapply Vars_decomp' in DE as DE'; simplify in DE'; cbn in DE'.
        destruct (IH (update x s var •₊₊ E') x) as [[sigma]|].
        + rewrite singlepoint_subst_Vars'; now eapply incl_cons_project_r in DE'.  
        + eapply DE'; intuition. 
        +  intros ? % singlepoint_subst_Vars'_variable; intuition. 
        + left; exists (update x (sigma • s) sigma); econstructor; eauto.
        + right; intros [sigma H]; inv H; [congruence|]; rewrite DE in *;
            injection H0 as ???; subst. 
          eapply f1; exists (sigma0); eauto.
    Qed.
  ...
  Section Unifiability.
    ...
    Proof.
      decomp_ind; eauto with equi_unifiable; intuition.
  ...
  Section Soundness.
    ...
    Proof.
      ...
      - intros H'; eapply decomp'_lambda_free in H; eauto. simplify in H. 
        intros y; unfold update; destruct eq_dec; subst; intuition.
    ...
    Proof.
      ...
      - eapply equi_unifiable_decomp' in H. eapply H.
        ...
        eapply Unifies_cons; cbn; intuition.
        + unfold unifies; cbn.
          unfold update at 1; destruct eq_dec; intuition.
    ...
    Proof.
      ...
      - intros LF T. eapply decomp'_typing in T as T'; eauto. 
        ...
        + eapply decomp'_lambda_free in H; eauto. simplify in H; cbn in H. intuition.
  ...
  Section Completeness.
    ...
    Proof.
      decomp_ind; intros; simplify in *; intuition.
    ...
    Proof.
      decomp_ind; cbn. all: intuition; simplify in *; try congruence.
    ...
    Proof.
      decomp_ind. all: intuition; simplify in *.
    ...
    Proof.
      ...
      - eapply decomp'_some_var in DE as H'; cbn in H'; destruct H' as [x ->].
        ...
        eapply unifies_not_var in H4 as H6;
          [| eapply decomp'_irrefl; eauto; intuition].
        ...
        + eapply decomp'_lambda_free in DE; eauto; simplify in DE; eauto.
          eapply lambda_free_subst_eqs; intuition; eauto.
  ...
  Section Equations.
    ...
    Lemma depdecomp': forall E, { o | decomp' E = o}.
    Proof.
      intros E; exists (decomp' E); reflexivity.
    Qed.

    Equations unif (E: list (exp X * exp X)) : option (fin -> exp X) by wf E subvars :=
      unif E with depdecomp' E => {
        unif E (exist _ (Some nil) _) := Some var;
        unif E (exist _ (Some ((var x, s) :: E')) _)
          with (x el vars s, isFree x, dec_all isFree (vars s)) => {
          unif E (exist _ (Some ((var x, s) :: E')) H) (right H1, left H2, left H3)
            with (unif (update x s var •₊₊ E')) => {
            unif E (exist _ (Some ((var x, s) :: E')) H) (right H1, left H2, left H3) (Some sigma)
            := Some (update x (sigma • s) sigma);
            unif _ _ _ _ := None
            };
          unif E _ _ := None
        }; 
    ...
    Lemma unif_correct E sigma:
      E ↦ sigma <-> unif E = Some sigma.
    Proof.
      split.
      - induction 1; simp unif; destruct depdecomp' as [o EQ]; rewrite EQ in H; subst.
        + reflexivity. 
        + cbn. destruct dec_in; intuition.
          destruct isFree; intuition.
          destruct dec_all; intuition.
          now rewrite IHunify.
      - revert sigma. funelim (unif E); try discriminate; injection 1 as <-.
...
Section FirstOrderDecidable.
  ...
  Proof.
    ...
    split; intros (Delta & sigma & ?); exists Delta; exists sigma; intuition; eauto 2 using FO_subst_equiv_eq, eq_equiv.
    ...
    destruct (k1 == k2) as [H4|H4];
      [subst; destruct (@unifiy_computable _ (fun x => x >= k2) _ [(AppR s T, AppR t T0)]) as [[sigma H']|H']|].
    ...
    1, 3: try (clear H'); eapply Arr_inversion in H3 as [? []]; try lia; rewrite ?H4;
      eauto; destruct x; cbn in *; simplify in H0; eauto;
    ...
    all: eapply order_one_lambda_free in N1 as L1; simplify; eauto.
    all: eapply order_one_lambda_free in N2 as L2; simplify; eauto.  
    + eapply unify_free' in H' as P.
      eapply unify_typing with (Gamma := rev L ++ Gamma) (L := [B]) in H' as T1;
        [| eauto|simplify; cbn; intuition; eauto using all_terms_nil|eauto using eqs_ordertyping].
      specialize (unify_lambda_free H') as LF; mp LF; simplify; intuition;
        eauto using all_terms_nil.
      exists Gamma. exists (decr (length L) sigma). intuition.
    ...
    + intros (Delta & sigma & ? & ? & ?).
      ...
      2: symmetry in H3.
      all: eapply Arr_inversion in H3 as [L'']; intuition; subst.
      ...
      2, 4: rewrite it_up_lt in H5; eauto; discriminate.
      1: inv H. 2: inv H2.
